...
def _calc_duration(date1, date2):
    return (datetime.strptime(date1, "%Y-%m-%d %H:%M:%S.%f") - datetime.strptime(date2, "%Y-%m-%d %H:%M:%S.%f")).total_seconds()
...
def pause(data, project_name, task_name):
    ...
    if data.get(project_name):
        if data.get(project_name)['tasks'].get(task_name):
            ...
            data.get(project_name)['tasks'][task_name]["duration"] += _calc_duration(f"{datetime.now()}", data.get(project_name)['tasks'][task_name]["since"])
...
def finish(data, project_name, task_name):
    ...
    if data.get(project_name):
        if data.get(project_name)['tasks'].get(task_name):
            ...
            data.get(project_name)['tasks'][task_name]["duration"] += _calc_duration(f"{datetime.now()}", data.get(project_name)['tasks'][task_name]["since"])
            data.get(project_name)['tasks'][task_name]["since"] = ""
            data.get(project_name)['tasks'][task_name]["finished_at"] = f"{datetime.now()}"
    return data
